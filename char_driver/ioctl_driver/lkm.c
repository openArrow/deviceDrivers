...
long ioctl(struct file *filp,unsigned int cmd,unsigned long arg)
{
	...
	case SCULL_IOC_SQUANTUM:
				...
				return EPERM;
				retval = __get_user(sculldev->quantum,(int __user*)arg);
				if(!retval)
				{	
					#ifdef DEBUG
					printk(KERN_INFO "get_user Failure\n");
					#endif
				}
				printk(KERN_INFO "quantum_size= %d\n",sculldev->quantum);	
				break;
	...
	case SCULL_IOC_TQUANTUM:
				if(! capable (CAP_SYS_ADMIN))
				return EPERM;
				retval = __put_user(sculldev->quantum,(int __user*)arg);
				if(!retval)
				{	
					#ifdef DEBUG
					printk(KERN_INFO "put_user Failure\n");
					#endif
				}
				break;
}

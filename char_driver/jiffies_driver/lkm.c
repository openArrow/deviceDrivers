...
int read_proc(char *page, char **start, off_t off,int count, int *eof, void *data)
{
	...
	start = &page;
	if(cycle == 0)
	count = sprintf(*start,"No cycle counter register \n");
	else
        {
        count = sprintf(*start,"No. of cycle \n");
        count+= sprintf(*start+count,"%ld\n",cycle);
	}
        count+= sprintf(*start+count,"TIME: \n");
        count+= sprintf(*start+count,"tv_sec=%u tv_usec=%u\n", tv.tv_sec, tv.tv_usec);
	count+= sprintf(*start+count,"this complexity of write function \n");
...
static void __exit hello_exit(void)
{
    ...
    //}
        remove_proc_entry("hello_proc_time",NULL);
#ifdef DEBUG
    printk(KERN_ALERT "Begins:%s\n", __func__);
#endif
#ifdef DEBUG
